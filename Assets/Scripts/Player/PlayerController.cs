...
public class PlayerController : MonoBehaviour
{
	...
	private void Start()
    {
		...
		gameAudioDevice = gameController.GetComponent<AudioSource>();
		if (isMultiplayer)
		{
	...
	private void Update()
	{
		if (!isMultiplayer || isMultiplayer && phView != null && phView.IsMine)
		{
		...
		if (interactDown && Time.timeScale != 0f)
		{
			...
			if (Physics.Raycast(ray, out raycastHit))
			{
				if (Vector3.Distance(raycastHit.transform.position, transform.position) < reachDist)
				{
					...
					else if (raycastHit.collider.gameObject.GetComponent<PlayerController>())
					{
