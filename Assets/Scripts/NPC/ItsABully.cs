...
public class ItsABully : MonoBehaviour
{
	...
	private void Update()
	{
        ...
        if (active)
		{
			...
			if (activeTime >= 180f & (transform.position - player.position).magnitude >= 60f)
			{
    ...
    private void OnTriggerStay(Collider other)
	{
		if (other.transform.name == "Principal" & guilt > 0f || other.transform.tag == "Player" && player.GetComponent<PlayerScript>().facultyTime > 0f)
		{
			...
			Reset();
			if (PlayerPrefs.GetInt("NoBullyingAchievement") != 1)
            {
		    	PlayerPrefs.SetInt("NoBullyingAchievement", 1);
				gameController.YTPGain(75);
				audioDevice.PlayOneShot(achievementSFX);
            }
		}
    }
    private void Reset()
