...
//DO NOT LOOK IN THIS CODE, IT IS PURE HELL!!!!!! or fix it for me
public class CJSMapEditor : EditorWindow
{
    ...
    private Transform GetSelectedTile()
    {
        ...
        {
            int index = 0; 
            if (hitColliders[index].gameObject.name == "TileGetter")
            {
                index = 1; 
            }
    ...
    private void RenderEditor_Edit()
    {
        ...
        if (currSelectionType == TileObject.Wall)
        {
            ...
            {
                if (currDecomp == Decomp.Simplified)
                {
                ...
                } else
                {
                    ...
                    {
                        StandardDoorScript doorCode = door.GetComponentInChildren<StandardDoorScript>();
                        doorCode.player = GameObject.FindGameObjectWithTag("Player").transform; //2019 support
                        SpoopBaldi baldi = FindObjectsOfTypeAll(typeof(SpoopBaldi))[0] as SpoopBaldi; //2019 support
                        doorCode.spoopBaldiScript = baldi; 
                    }
